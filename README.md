...
응답 메시지 끝에 사용량 요약이 자동으로 포함됩니다.

```
📊 Opus 4.6 | Tokens In: 17,410 Out: 523 (cache hit 65%) | Cost: $0.0442 | Time: 2.7s
```

- 사용된 모델명, 입출력 토큰 수, 캐시 히트율, 비용, 소요 시간을 한눈에 확인할 수 있습니다.

### 모델 및 Effort 설정

스레드 내에서 `!` 명령어로 모델과 effort 레벨을 변경할 수 있습니다.
...
| `!default model sonnet` | 기본 모델 변경 (전체 적용, 영구 저장) |
| `!default effort high` | 기본 effort 변경 (전체 적용, 영구 저장) |

- 기본값: **sonnet** / **high** (Claude Code 기본과 동일)
- 프로젝트 선택 시 현재 모델/effort 설정이 표시됩니다.
...
스레드 내 아무 메시지에 ❌ 리액션을 달면 진행 중인 Claude 작업을 중단할 수 있습니다.

- Ctrl+C와 동일한 SIGINT를 전송하여 세션 상태가 보존됩니다 (이후 resume 가능).
- 10초 내 종료되지 않으면 SIGKILL로 강제 종료합니다.
- Slack 앱 설정에서 `reaction_added` 이벤트 구독이 필요합니다.
...
- **`--resume`으로 이전 대화 이어가기** — 데몬 재시작 후에도 기존 스레드에서 대화를 계속할 수 있습니다. Claude의 `stream-json` init 이벤트에서 세션 ID를 자동 캡처하여 저장합니다.
- **thread→project 매핑 유지** — 어떤 스레드가 어떤 프로젝트에 연결되어 있는지 기억합니다.
- **모델/effort 설정 유지** — 글로벌 기본값과 스레드별 설정이 저장됩니다.
...
│   ├── claude_handler.py   # Claude CLI 호출, 모델/effort 설정, 사용량 파싱
│   ├── slack_daemon.py    # Slack Socket Mode + Unix 소켓 서버 + ! 명령어
│   ├── session_broker.py  # Unix 소켓 클라이언트 — 메시지 게시, 답변 대기
│   ├── mcp_server.py      # MCP 도구 등록 (ask/notify/upload/download)
│   ├── tools_mcp.py       # Slack→Claude 방향 MCP 도구 (notify/upload/download)
│   ├── event_poster.py    # 실시간 진행 상황 Slack 포스팅
│   ├── constants.py       # 공유 상수 (모델, effort, 소켓 경로 등)
